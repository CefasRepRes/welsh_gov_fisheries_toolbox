...
## Explore the stats from EFLALO to get fleet statistics and fleet characterization
  ...
  distinct(eflalo, FT_REF)%>%dim() 
  distinct(eflalo_gbw, FT_REF)%>%dim() 
  
  ## We need to filter the TACSAT data that is related to the filtered trips 
  
  tacsat_gbw = tacsat%>%filter(SI_FT %in% ( eflalo_gbw%>%select(FT_REF)%>%pull()))
  ...
  eflalo_gbw$VE_LEN_CAT=  cut(as.numeric(eflalo_gbw$VE_LEN), include.lowest = T,
                          breaks=c(-Inf, 4.5, 5 , 6, 7, 8, 9, 10 , Inf), 
                          labels=c("<4.5 m","4.5. m", "5 m","6 m", "7 m", "8 m", "9 m", "=> 10  m"))
  ...
  ## Plot the categories to understand your fleet composition
    ...
    ggplot(eflalo_gbw%>%distinct(FT_REF, VE_LEN_CAT), aes(VE_LEN_CAT)) + geom_bar()
    
    ## PLOT 2: Number of trips by gear category 
    
    ggplot(eflalo_gbw%>%distinct(FT_REF, LE_GEAR), aes(LE_GEAR)) + geom_bar()
    
    ## PLOT 2: Number of trips by gear category 
    
    ggplot(eflalo_gbw%>%distinct(FT_REF,VE_LEN_CAT, LE_GEAR), aes(LE_GEAR)) + geom_bar() + facet_wrap( ~ VE_LEN_CAT)
    
    ## PLOT 3: Main species captured
    
      ## Rank the main species captured by gear type
    
    eflalo_gbw%>%group_by(  LE_GEAR, LE_SPE) %>%
    ...
    ggplot(., aes(y =le_kg_total, x= LE_SPE)) + geom_bar(stat = "identity") + facet_wrap(~LE_GEAR, scales = "free")
    ...
    ggplot(eflalo_gbw%>%filter(trip_days < 5)) + geom_histogram(aes(trip_days) )
    
    ggplot(eflalo_gbw%>%filter(trip_days < 50), aes(trip_days, VE_LEN )) + geom_point(  )
