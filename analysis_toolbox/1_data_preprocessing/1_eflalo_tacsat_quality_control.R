...
eflalo_fs %>%
  filter(FT_REF %in% trips_with_more_than_1_gear$FT_REF) %>%
  group_by(FT_REF) %>%
  mutate(LE_KG_TOT = sum(LE_KG)) %>%
  group_by(FT_REF, LE_GEAR) %>%
  reframe(LE_KG_TOT = LE_KG_TOT, LE_KG_TOT_GEAR = sum(LE_KG)) %>%
  distinct(FT_REF, LE_GEAR, LE_KG_TOT, LE_KG_TOT_GEAR) %>%
  mutate(LE_KT_PROP = LE_KG_TOT_GEAR / LE_KG_TOT) %>%
  print(n = 110)
...
tacsat_sel = tacsat_fs %>%
  filter(SI_FT == 610715898 & SI_SP >= 0 & SI_SP <= 5) %>%
  arrange(SI_DATIM) %>%
  left_join(eflalo_sel %>% filter(FT_REF == 610715898), by = c("SI_FT" = "FT_REF", "SI_DATE" = "LE_CDAT"))
...
aoi = st_bbox(c(xmin = -15, xmax = 3, ymax = 60, ymin = 47), crs = st_crs(4326)) ## Define our area of intenrest. 4326 is the id for WGS1984 unprojected coordinate system
...
trips_in_clean_eflalo = eflalo_fs %>%
  distinct(FT_REF) %>%
  pull()
...
tacsat_fs -> bk
tacsat_fs = tacsat_fs %>%
  filter(SI_FT %in%  trips_in_clean_eflalo)


# Q1 : Number of trips in tacsat (iVMS) present in eflalo

tacsat_fs %>%
  mutate(inboth = SI_FT %in% trips_in_clean_eflalo) %>%
  select(inboth) %>%
  table()
...
tacsat_fs_geom %>%
  filter(VE_REF == "C20757" & SI_FT == "610808700") %>%
  arrange(SI_DATIM)
...
tacsat_fs_geom %>%
  ...
  ggplot() +
  geom_sf(aes(color = largeIntv)) +
  geom_sf_label(aes(label = ifelse(INTV > 30, round(INTV, 1), NA)),  nudge_x = 0.05) +
  theme_minimal()
...
ggplot() +
  ...
  geom_sf(data = tacsat_fs_land %>%
            slice(1:50000), aes(color  = SI_LAND)) +
  theme_minimal() +
  coord_sf(xlim = c(- 5.5, -4), ylim = c(51.6, 52.2))
