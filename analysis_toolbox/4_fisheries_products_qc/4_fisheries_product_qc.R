...
for (nn in colnames(table1Save)) {
  ...
  row <- c(field = nn, is_na =  ifelse(is.na(table_na["TRUE"]), 0, table_na["TRUE"]), total_records =  table1Save[, nn] %>%
             length(), field_type = class(table1Save[, nn]))
...
for (nn in colnames(table2Save)) {
  ...
  row <- c(field = nn, is_na =  ifelse(is.na(table_na["TRUE"]), 0, table_na["TRUE"]), total_records =  table2Save[, nn] %>%
             length(), field_type = class(table2Save[, nn]))
  table_nas <- rbind(table_nas,  row)
}
...
write.table(table1Save, file.path(outPath, "table1Save_", analysis_type, ".csv"), na = "", row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
write.table(table2Save, file.path(outPath, "table2Save_", analysis_type, ".csv"), na = "", row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
...
screen_vms_file(file.path(outPath, "table1Save_", analysis_type, ".csv"))  # Submit for screening Table 1
screen_vms_file(file.path(outPath, "table2Save_", analysis_type, ".csv"))  # Submit for screening Table 2
