...
for (year in years) {
  ...
  if (analysis_type == "welsh_fleet")  {
    ...
    data_folder_t3 = file.path(here("data/welsh_fleet_data/t3"))
    data_folder_geofish = file.path(here("data/welsh_fleet_data/geofish/", year))
    ...
    data_folder_t3 = file.path(here("data/welsh_waters_data/t3"))
    data_folder_geofish = file.path(here("data/welsh_waters_data/geofish/", year))
  ...
  if (year == "2022") {
    ...
    eflalo_ft_t3 = read.csv(file = paste0(data_folder_t3, "\\eflalo_ft.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM")
    eflalo_ft_gf = read.csv(file = paste0(data_folder_geofish, "\\eflalo_ft.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM")
    ...
    eflalo_le_t3 = read.csv(file = paste0(data_folder_t3, "\\eflalo_le.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM", colClasses = c(rep(NA, 10), "character"))
    eflalo_le_gf = read.csv(file = paste0(data_folder_geofish, "\\eflalo_le.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM", colClasses = c(rep(NA, 10), "character"))
    ...
    eflalo_spe_t3 = read.csv(file = paste0(data_folder_t3, "\\eflalo_spe.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM")
    eflalo_spe_gf = read.csv(file = paste0(data_folder_geofish, "\\eflalo_spe.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM")
  ...
  if (year == 2022) {
    ...
    tacsat_gf = read.csv(file = paste0(data_folder_geofish, "\\tacsat.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM")
  ...
  if (fleet_segment == "over12")  {
    ...
    eflalo_fs$VE_LEN_CAT = cut(eflalo_fs$VE_LEN, include.lowest = TRUE,
  ...
  trips_mult_gears_comb = eflalo_fs_mult_gears %>%
    distinct(FT_REF, LE_GEAR, LE_MET) %>%
    arrange(FT_REF, LE_GEAR, LE_MET) %>%
    group_by(FT_REF) %>%
  ...
  overlaps_analysis =  eflalo_fs %>%
    distinct(VE_REF, FT_REF, FT_DDATIM, FT_LDATIM) %>%
  ...
  ft_ref_not_in_tacsat = eflalo_fs %>%
    distinct(FT_REF) %>%
    mutate(inboth = FT_REF %in% (tacsat_fs %>%
                                   distinct(SI_FT) %>%
                                   pull())) %>%
    filter(inboth == FALSE)

  res1 = eflalo_fs %>%
    filter(FT_REF %in% (ft_ref_not_in_tacsat %>%
                          select(FT_REF) %>%
                          pull())) %>%
  ...
  tacsat_fs_geom = tacsat_fs_geom %>%
    ungroup() %>%
    group_by(VE_REF, SI_FT) %>%
    arrange(VE_REF, SI_DATIM) %>%
    ...
    mutate(interval_mean = mean(INTV, na.rm = TRUE)) %>% ## Calcualte the mean to replace the NA"s interval when a VMS location is the 1st of a trip and cannot calculate with a prev. iVMS location
  ...
  tacsat_fs_ports = tacsat_fs_geom %>%
    st_join(port_500m %>%
              select(Name, District_N), join = st_intersects, left = TRUE) %>%
    mutate(SI_HARB = ifelse(is.na(Name), FALSE, TRUE)) %>%
    select(- names(port_500m %>%
  ...
  tacsat_fs_land = tacsat_fs_ports %>%
    st_join(europe_aoi, join = st_intersects, left = TRUE) %>%
  ...
  tacsat_fs_df_geom = tac_geom %>%
    st_join(ices_rect_welsh %>%
              select(icesname), join = st_intersects, left = TRUE) %>%
  ...
  ft = eflalo_sel %>%
    arrange(FT_REF, LE_CDAT) %>%
    select(FT_REF, LE_GEAR) %>%
    rename(LE_GEAR2 = LE_GEAR) %>%
    group_by(FT_REF) %>%
    slice(1)
  ...
  tacsatp = tacsatp %>%
    left_join(ft, by = c("SI_FT" = "FT_REF")) %>%
  ...
  tacsatp = tacsatp %>%
    left_join(ve_c, by = c("SI_FT" = "FT_REF")) %>%
...
for (year in years) {
  ...
  if (eflalo_format == "wide") {
    ...
    if (species_analysis_type == "all_species_sum") {
      ...
      eflalo_fs_tot = eflalo_fs %>%
        mutate(LE_KG_TOT = select(., idxkg) %>%
                 rowSums(., na.rm = TRUE)) %>%
        select(-c(idxkg, idxval)) %>%
        mutate(LE_EURO_TOT = NA) %>%
        as.data.frame()
      ...
      eflalo_fs_tot = eflalo_fs %>%
        mutate(LE_KG_TOT = select(., contains(c("KG_SOL", "KG_MAC"))) %>%
                 rowSums(., na.rm = TRUE)) %>%
        select(-c(idxkg, idxval)) %>%
        as.data.frame()
      ...
      eflalo_fs_tot = eflalo_fs %>%
        select(., -idxkg, -idxval, contains(c("SOL", "MAC"))) %>%
        filter_at(vars(contains(c("SOL", "MAC"))), any_vars(!is.na(.))) %>%
        as.data.frame()
    ...
    captures_total = eflalo_fs %>%
      group_by(FT_REF, LE_ID) %>%
      summarise(LE_KG_TOT = sum(LE_KG), LE_EURO_TOT = 0) # LE_KG_TOT = sum( LE_KG) if VALUE data is available


    eflalo_fs_tot = eflalo_fs %>%
      select(-c(LE_SPE, LE_KG, LE_VALUE)) %>%
      distinct() %>%
      inner_join(captures_total, by = c("FT_REF", "LE_ID"))
  ...
  eflalo_output = eflalo_output %>%
    group_by(VE_REF, LE_CDAT) %>%
    mutate(nr = row_number()) %>%
    mutate(nr = max(nr)) %>%
    mutate(INTV = INTV / nr) %>%
    arrange(VE_REF, LE_CDAT) %>%
    as.data.frame()
