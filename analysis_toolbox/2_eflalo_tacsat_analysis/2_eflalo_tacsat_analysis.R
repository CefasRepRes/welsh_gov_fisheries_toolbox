...
ve_c = eflalo_sel %>%
  distinct(FT_REF, VE_LEN, VE_KW, VE_COU) %>%
  rename(VE_LEN2 = VE_LEN, VE_KW2 = VE_KW, VE_COU2 = VE_COU)
...
tacsatp = tacsatp %>%
  ...
  mutate(interval_mean = mean(INTV, na.rm = TRUE)) %>%                           ## Calculate the mean to replace the NA"s interval when a VMS location is the 1st of a trip and cannot calculate with a prev. iVMS location
...
tacsatp %>%
  group_by(SI_FT) %>%
  mutate(mean_intv_trip = mean(INTV)) %>%
...
ggplot(data = tacsatpgf, aes(SI_SP, fill = SI_STATE)) +
  geom_histogram(breaks = seq(0, 10, by = 0.4), col = 1) +
...
if (!"MIS" %in%  unique(speedarr$LE_GEAR)) {
  speedarr =  speedarr %>% rbind(c("MIS", NA, NA))
}
...
# subTacsat$ID <- seq_len(nrow(subTacsat))

subTacsat %>%
  group_by(LE_GEAR, SI_STATE) %>%
  summarise(f_min_s = min(SI_SP), f_max_s = max(SI_SP))

ggplot(subTacsat %>%
         filter(LE_GEAR == "OTB"), aes(x = SI_SP, fill = SI_STATE)) +
  geom_bar() +
  theme_minimal()
...
save(tacsatEflalo, file = paste0(here(), "\\data\\workflow_outputs\\tacsatEflalo_output_", analysis_type, "_", year, ".RData"))
