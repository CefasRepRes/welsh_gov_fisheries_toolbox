...
library(here)       ## Package for referring back to root of git
...
## Filter GeoFISH data for Over 10 meter vessels. Logbook info from <10 m in GeoFISH it comes from Sales Notes and is less reliable.
...
# explore the loaded data, can change to geofish data to check
head(eflalo_ft_t3)
str(eflalo_ft_t3)
dim(eflalo_ft_t3)
...
eflalo$FT_DDAT = ymd(eflalo$FT_DDAT)  ## ymd lubridate function to CAST date into year-month-date date format
eflalo$FT_LDAT = ymd(eflalo$FT_LDAT)

eflalo = eflalo %>% mutate(LE_CDAT = substr(eflalo$LE_CDAT, 1, 10))  ## To convert Log Event date to year-month-day. If time is not provided, by default it will be set to 00:00:00
...
tacsat_t3 = read.csv(file = paste0(data_folder_t3, "\\tacsat.csv"), header = TRUE, sep = ",", fileEncoding = "UTF-8-BOM")
...
save(eflalo, file = paste0(here(), "\\data\\workflow_outputs\\eflalo_", analysis_type, ".RData"))

# save(tacsat, file = ".\\workflow_outputs\\tacsat_ww.RData")
save(tacsat, file = paste0(here(), "\\data\\workflow_outputs\\tacsat_", analysis_type, ".RData"))
